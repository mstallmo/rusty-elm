...
Elm$ReasonableRustyElm.newApp.ports.renderLayers.subscribe((function (layers) {
        console.log(layers);
        var firstLayer = Caml_array.caml_array_get(layers, 0);
        var firstClampedArrayBuffer = fillClampedArrayFromArray(firstLayer.image, new Uint8ClampedArray(firstLayer.image.length));
        Curry._2(Render$ReasonableRustyElm.renderPsd, firstLayer.name, new ImageData(firstClampedArrayBuffer, firstLayer.width, firstLayer.height));
        var secondLayer = Caml_array.caml_array_get(layers, 1);
        var secondClampedArrayBuffer = fillClampedArrayFromArray(secondLayer.image, new Uint8ClampedArray(1000000));
        return Curry._2(Render$ReasonableRustyElm.renderPsd, secondLayer.name, new ImageData(secondClampedArrayBuffer, 500, 500));
      }));
